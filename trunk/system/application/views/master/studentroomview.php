...
<form name="rimba" method="post" action="<?=site_url("master/studentroom/save");?>">
		<script>
		
			function changeClass(url) {
				window.location="<?=site_url("master/studentroom/listbyroom");?>"+"/roomid/"+url;
			}
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<select name="roomidfrom" onChange="changeClass(this.options[this.selectedIndex].value)">
			...
			?>
			<option  <?if ($roomid == $item->id){?>selected<?}?>  label="<?=$item->name?> (<?=$item->parent?>)" value="<?=$item->id?>"><?=$item->name?> (<?=$item->parent?>)</option>
			<?}?>
		</select>
		...
		<select name="roomidto">
			<option></option>			
				<?  
					...
					foreach($roomList as $item) {
				?>
				<option label="<?=$item->name?> (<?=$item->parent?>)" value="<?=$item->id?>"><?=$item->name?> (<?=$item->parent?>)</option>
				<?}?>
		</select>
			</td>
        </tr>
    ...
    <td align="right">
		...
		<input type="button" class="formbox" value=" Refresh " OnClick="javascript:window.location='<?=site_url("master/studentroom/liststudentroom");?>'">
			...
			?>
				<tr>
					<td width="20%" class="kiritabel">&nbsp;
						<input type="checkbox" name="studentid[]" value="<?=$item->id?>">
					</td>
					<td width="30%" class="isitabel">&nbsp;<?=$item->regno?></td>
