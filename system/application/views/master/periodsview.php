...
<table width="95%" border="0" cellspacing="0" cellpadding="0" align="center">
  ...
  <tr> 
    <td> 
      &nbsp;
<form name="periods" method="post" action="<?=site_url("master/periods/save");?>">
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" dwcopytype="CopyTableCell">
        <tr> 
          ...
          <td width="32%" class="formcolor"> 
            <input type="text" name="begin" size="25" value="<?=$periodsData['begin']?>"> (yyyy-mm-dd)
          </td>
        </tr>
        <tr> 
          ...
          <td width="32%" class="formcolor"> 
            <input type="text" name="end" size="25" value="<?=$periodsData['end']?>" > (yyyy-mm-dd)
          </td>
        </tr>
        <tr> 
          <td width="18%" class="formcolor">&nbsp;</td>
          <td width="32%" class="formcolor"> 
            <input type="checkbox" name="active" value="1" <?if(!empty($periodsData['active'])){echo "checked=1";}?> > Aktif (Semester Berjalan) 
            <input type="hidden"  name="periodsid" value="<?=$periodsData['id']?>">
          </td>
        </tr>
  ...
  <tr> 
    <td align="right">
	 	<input type="button" class="formbox" value=" Save " OnClick="javascript:this.form.submit()">
                <input type="button" class="formbox" value=" Refresh " OnClick="javascript:window.location='<?=site_url("master/periods/listperiods");?>'">	 	
  ...
  <tr>
    <td><!-- #BeginEditable "table" -->
        <table width="100%" border="0" cellpadding="0" cellspacing="0">
            ...
            <?  
                $line=0;
                foreach($periodsList as $item) {
            ?>	         
            <tr> 
                <td class="kiritabel">&nbsp;<?=++$line?></td>
                <td class="isitabel">&nbsp;
                    <a href="<?=site_url("master/periods/edit/periodsid/".$item->id);?>"><?=$item->begin." - ".$item->end ?></a>&nbsp;&nbsp;
                </td>
                <td class="isitabel">&nbsp;
                    <?
                        if ($item->active == 1){
                ...
                <td><a href="<?=site_url("master/periods/remove/periodsid/".$item->id);?>">(hapus)</a></td>
            </tr>
            <? }?>
        </table>
      <!-- #EndEditable --></td>
